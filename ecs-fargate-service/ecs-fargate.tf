...
resource "aws_lb_target_group" "blue" {
  ...
  name                 = length(var.name_prefix) > 27 ? concat([substr(var.name_prefix, 0, 27), "-blue"]) : "${var.name_prefix}-blue"
  vpc_id               = var.vpc_id
...
resource "aws_lb_target_group" "green" {
  ...
  # TG name cannot be longer than 32 characters
  name                 = length(var.name_prefix) > 26 ? concat([substr(var.name_prefix, 0, 26), "-green"]) : "${var.name_prefix}-green"
  vpc_id               = var.vpc_id
  protocol             = var.task_container_protocol
  port                 = var.task_container_port
  target_type          = "ip"
